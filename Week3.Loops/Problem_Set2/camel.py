...
# for variables’ names when those names comprise multiple words,
# whereby the first letter of the first word is lowercase
# but the first letter of each subsequent word is uppercase.
# For instance, whereas a variable for a user’s name might be called name,
...
#Python, by contrast, recommends snake case,
# whereby words are instead separated by underscores (_),
...
def main():
    camel_in = camel()
    snake_out = snake(camel_in)
    print("snake_case: ", snake_out)
...
def snake(y):
    for letter in y:
        if letter.isupper() == True:
            snakeletter = ("_" + letter.lower())
            y = y.replace(letter, snakeletter)
    return y
